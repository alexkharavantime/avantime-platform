...
| Jira               | existing client                                     | placeholders only, disabled until TASK-016                      | Jira Cloud later                               | disabled flag, non-secret URL/project       | TASK-016 ticket workflow                         | intentionally out of scope      | credentials absent, flag false               |

## Topology

`docker-compose.staging.yml` — единственный staging deployment manifest. Он запускает migration
job, notification worker, knowledge index worker, web и опциональный backup job. Services работают
...
backup/observability references и disabled Jira credentials. Managed mode дополнительно требует
TLS и реальный notification provider; local/test adapters запрещены. Development fallback нет.
...
- `/ready` проверяет configuration, DB, migrations, Redis policy, object probe, provider adapter,
  worker generations, pgvector/index tables и governance invariants;
