...
2. **SEC-001:** добавить автоматические отрицательные security-тесты и завершить критерии готовности;
...
| Последний commit | `dd91b55 chore: add Codex guidelines and local data exclusions` |
...
| Безопасность | In Progress | 45% | Dashboard и внутренние AI/document API закрыты; обращения и вложения изолированы по компании, но полная RBAC-модель и tenant-документы отсутствуют |
...
| Тестирование | Planned | 10% | Формальная стратегия и достаточное автоматическое покрытие отсутствуют |
...
- автоматические security-тесты для dashboard, document API и AI API;
...
- удалённый в рабочем дереве маршрут `apps/web/app/knowledge/[slug]/page.tsx` ломает открытие публичных статей;
- `apps/web/next.config.ts` меняет `transpilePackages` без подтверждённой проверки UI workspace;
- перенос `packages/ui/src/button.tsx` в `packages/ui/src/components` не завершён и не проверен;
- OpenAI установлен в корневом workspace, хотя используется web-приложением;
- demo session secret используется при отсутствии production-конфигурации;
- тесты и production checks для текущего рабочего дерева не подтверждены.
...
**Комментарий:** dashboard и внутренние AI/document routes требуют серверную сессию; document API временно ограничен `ADMIN`, обращения и вложения фильтруются по `companyId`, а логирование части AI-ключа удалено. Полная ролевая модель и tenant-метаданные документов отсутствуют.
...
**Комментарий:** формальная стратегия тестирования и подтверждённое покрытие критических сценариев отсутствуют.
...
| Количество страниц | 31 | Фактические `page.tsx` в `apps/web/app`; удалённый tracked-файл не учитывается |
...
| Количество известных ошибок | 6 | Явно перечисленные проблемы в разделе «Технический долг»; число требует обновления после тестирования |
...
| 2 | Восстановить или осознанно заменить публичный маршрут статьи | P0 | Устранение пользовательской регрессии | Низкая |
| 3 | Добавить security-тесты для dashboard и закрытых API | P0 | Подтверждение реализованной защиты от регрессий | Средняя |
| 4 | Добавить tenant-метаданные документам | P0 | Безопасный доступ клиентов к собственным документам | Высокая |
| 5 | Утвердить RBAC и tenant-контекст | P0 | Основа безопасного портала и мультитенантности | Высокая |
| 6 | Создать единый Storage Adapter | P0 | Безопасные production-документы и масштабирование | Высокая |
| 7 | Создать внутренний AI Gateway | P0 | Единые политики, стоимость, аудит и модели | Высокая |
| 8 | Объединить `/portal` и `/dashboard` по этапам | P0 | Цельный UX и устранение дублирования | Высокая |
| 9 | Внедрить очереди, мониторинг и корреляционные ID | P0 | Надёжная обработка и диагностика | Высокая |
| 10 | Добавить security и integration tests критических сценариев | P0 | Проверяемая готовность Version 2.0 | Высокая |
